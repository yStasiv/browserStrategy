...
class HelperMethods:
    ...
    def update_user_level(user: models.User, db: Session):
        ...
        next_level_exp = calc.CalsculateExpForNextLvl.experience_needed_for_next_level(user.level)
        try:
...
def get_available_units(level: int, db: Session):
    ...
    return db.query(models.UnitType).filter(models.UnitType.level_required <= level).all()
