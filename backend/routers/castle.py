...
class CastleHelpers:
    ...
    def get_available_units(user: models.User, db: Session):
        ...
        logger.info(
            f"Fetching available units for user id {user.id}, level {user.level}, fraction {user.fraction}"
        )
        unit_types = (
        ...
        if not unit_types:
            logger.warning(
                f"No units found for user id {user.id}, level {user.level}, fraction {user.fraction}"
            )
        else:
            for unit in unit_types:
                logger.info(
                    f"Found unit: {unit.name}, level_required: {unit.level_required}"
                )
    ...
    def add_units_to_user(
        user: models.User, unit_type_id: int, quantity: int, db: Session
    ):
        ...
        if quantity < 0 or quantity > unit_type.max_quantity:
            logger.error(
                f"Unit '{unit_type.name}' quantity should be between 0 and {unit_type.max_quantity}, actual quantity is {quantity}"
            )
            return
...
class CastleRotes(CastleHelpers):
    ...
    async def add_units(request: Request, db: Session = Depends(database.get_db)):
        ...
        for unit in form_data:
            ...
            unit_type = (
                db.query(models.UnitType)
                .filter(models.UnitType.name == unit_name)
                .first()
            )
