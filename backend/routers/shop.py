...
async def shop_page(request: Request, db: Session = Depends(database.get_db)):
    ...
    shop_items = db.query(models.ShopItem).filter(
        models.ShopItem.level_required <= user.level
    ).all()
...
async def buy_item(
    ...
    if user.level < shop_item.level_required:
        raise HTTPException(
            status_code=400, 
            detail=f"Required level: {shop_item.level_required}"
        )
    ...
    inventory_count = db.query(models.Item).filter(
