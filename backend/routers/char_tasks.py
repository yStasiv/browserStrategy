...
class TaskHelper:
    ...
    @staticmethod
    def update_tasks_for_users(
        db: Session, db_task: models.Task, all_users: list[models.User]
    ) -> None:
    ...
    def create_task(
        ...
        # Add this task for each users
        TaskHelper.update_tasks_for_users(
            db, db_task, all_users=db.query(models.User).all()
        )
        return db_task
    ...
    def get_available_tasks_for_user(db: Session, user: models.User):
        tasks = (
            ...
            .filter(
                ...
                models.Task.level_required <= user.level,
            )
            .all()
        )
        if user.id == 1:
    ...
    def update_task_completion(
        db: Session, user_id: int, task_id: int, is_completed: bool
    ):
        user_task = (
            db.query(models.UserTask)
            .filter(
                models.UserTask.user_id == user_id, models.UserTask.task_id == task_id
            )
            .first()
        )
        if user_task:
            ...
            if task and is_completed:
                ...
                if reward_exp:
                    logger.info(
                        f"Task retun some exp {reward_exp}, let up user lvl if needed"
                    )
                    user = (
        ...
        if task_id == 1:
            ...
            if not existing_achievement:
                user_achievement = models.UserAchievement(
                    user_id=current_user.id, achievement_id=1
                )
