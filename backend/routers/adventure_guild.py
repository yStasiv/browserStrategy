...
async def get_guild(
    ...
    if scenario_id:
        available_tasks = (
            ...
            .order_by(Task.order_in_scenario)
            .all()
        )
    else:
...
async def accept_task(
    ...
    existing_task = (
        db.query(UserTask)
        .filter(UserTask.user_id == current_user.id, UserTask.task_id == task_id)
        .first()
    )
    ...
    user_task = UserTask(user_id=current_user.id, task_id=task_id, is_completed=False)
